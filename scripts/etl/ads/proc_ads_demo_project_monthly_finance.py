"""Sanitized demo project-month finance mart refresh."""
...
DATABASE = "demo_ads"
TARGET_TABLE = "ads_demo_project_monthly_finance"
...
def build_insert_sql(target_table: str, biz_date: str) -> str:
    """Build the ADS full rolling-recalculation SQL."""
    biz_month = biz_month_sql(biz_date)
    return f"""
    ...
    SELECT
        ...
        ROUND(net_receivable_amount / NULLIF(order_count, 0), 2)
            AS average_order_receivable_amount,
        NOW() AS etl_time
    FROM demo_dws.dws_demo_project_monthly_receivable
    WHERE stat_month <= {biz_month};
    """.strip()


def build_refresh_sql(biz_date: str, *, run_token: str = "dryrun") -> str:
    """Render the ADS shadow-table refresh plan."""
...
def run(biz_date: str) -> None:
    """Publish the ADS rolling-recalculation snapshot."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Refresh the sanitized demo monthly finance mart")
